...
# 尝试从 Streamlit 获取秘密信息
try:
    import streamlit as st
    # 优先从 Streamlit Secrets 读取
    UUID = st.secrets.get("MY_UUID", "")
    PORT = int(st.secrets.get("MY_PORT", 49999))
    DOMAIN = st.secrets.get("MY_DOMAIN", "")
    TOKEN = st.secrets.get("MY_TOKEN", "")
except (ImportError, Exception):
    # 如果不是在 Streamlit 环境运行，或者 Secrets 没配置，则使用以下默认值
    # 建议在公开代码时，把这里的具体数值删掉或改成空字符串 ""
    UUID = "" 
    PORT = 0
    DOMAIN = ""
    TOKEN = ""
